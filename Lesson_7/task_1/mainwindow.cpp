    ...
    , ui(new Ui::MainWindow)
{
    ...
    // connect(&whFutReadData, &QFutureWatcher<QVector<uint32_t>>::finished, this, [this]{
    ...
    connect(this, &MainWindow::sig_resultReady, this, [this]{
        DisplayResult(mins, maxs);
    });
    ...
    connect(this,&MainWindow::sig_graphReady,this,&MainWindow::slot_graph_replot);
...
{    
    ...
    foreach (double num, resultData){
        ...
        if(count<1000){
            x[count]=count/1000.0;
            y[count]=num;
            graph->addData(x[count],y[count]);
            ++count;
        }
    }
...
void MainWindow::on_pb_start_clicked()
{
    ...
    */
    ...
    auto proc = [this](QVector<uint32_t> readData){return ProcessFile(readData);};
    auto fins = [this](QVector<double> procData){

        maxs = FindMax(procData);
        mins = FindMin(procData);
        emit sig_resultReady( );

    };
    auto graph_replot=[this]{emit sig_graphReady();};

    QFuture<void> fut = QtConcurrent::run(read).then(proc).then(fins).then(graph_replot);
...
void MainWindow::slot_graph_replot(){
